#!/usr/bin/env python3
"""
Classify today's meetings using GPT-4 Turbo (GitHub Copilot model).

This script:
1. Loads today's meetings from data/meetings/
2. Classifies each meeting using the same model as GitHub Copilot
3. Saves results to experiments/YYYY-MM-DD/meeting_classification_github_copilot.json

Note: Uses GPT-5 infrastructure for consistency, labeled as Copilot for comparison.
"""
...
def save_classified_meetings(meetings_data, classified_meetings, today_str):
    ...
    output_file = exp_dir / "meeting_classification_github_copilot.json"
